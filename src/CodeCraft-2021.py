...
def readdata():
    ...
    for i in range(serv_num):
        ...
        serv.append(line)
    serv.sort(key=lambda x: (x[3] + x[4])//(x[1] + x[2]))
    for ser in serv:
...
def main():
    ...
    for i in range(Days):
        ...
        for req in Reqs[i]:
            ...
            if req[0] == 'add':
                for k, v in serv_poll.items():    # serv_poll[NUM] = [ser[0], ser[1] / 2, ser[2] / 2, ser[1] / 2, ser[2] / 2]
                    if vm[req[1]][2] == 1:                      # 双节点部署
                        ...
                        # 判断节点是否可以部署
                        if v[1] >= vm[req[1]][0] and \
                                v[2] >= vm[req[1]][1] and \
                                value_rd(vm[req[1]][3], round(v[1]/v[2], 1), 0.3):
                ...
                if flag == 0:
                    ...
                    while True:
                        flag1 = 0           # 标志是否扩容成功
                        for ser in serv:
                            if value_rd(vm[req[1]][3], ser[5], r) and \
